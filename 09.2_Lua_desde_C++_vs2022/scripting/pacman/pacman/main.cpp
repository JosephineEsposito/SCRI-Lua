...
{ // Pacman se ha comido un powerUp
	...
	if (L)
	{
		...
		if (lua_pcall(L, 1, 1, 0) != 0)
		{
			...
			if (lua_istable(L, -1)) { // Controlla se il valore di ritorno è una tabella
				int r, g, b;

				// Ottieni i valori RGB dalla tabella
				lua_pushstring(L, "r"); // Push della chiave "r"
				lua_gettable(L, -2);    // Ottieni il valore associato alla chiave "r" dalla tabella (stack index -2)
				r = static_cast<int>(lua_tonumber(L, -1)); // Converti in numero e poi in int
				lua_pop(L, 1);          // Rimuovi il valore 'r' dallo stack

				lua_pushstring(L, "g"); // Push della chiave "g"
				lua_gettable(L, -2);    // Ottieni il valore associato alla chiave "g"
				g = static_cast<int>(lua_tonumber(L, -1));
				lua_pop(L, 1);

				lua_pushstring(L, "b"); // Push della chiave "b"
				lua_gettable(L, -2);    // Ottieni il valore associato alla chiave "b"
				b = static_cast<int>(lua_tonumber(L, -1));
				lua_pop(L, 1);

				// Rimuovi la tabella dallo stack
				lua_pop(L, 1);

				// Applica il colore a Pac-Man
...
bool InitGame()
{
	...
	{
		fprintf(stderr, "Error with loading or executing pacman.lua: %s\n", lua_tostring(L, -1));
		return false;
	}
